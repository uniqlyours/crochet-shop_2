...
// ---------- social posting queue (Instagram) ----------
// Ships with studio-made tip cards and reels; admin can add more via the
// Instagram page in the admin panel. The queue cycles forever.
const HT = '\n\n#crochet #handmade #crochetlove #handmadewithlove #shopsmall #crochetersofinstagram #uniqlyours';
const SEED_SOCIAL = [
  { id: 's_tip01', type: 'image', file: '/uploads/social/tip-01.jpg', caption: 'Maker note Nº01 — straight edges, every time. Count your rows, mark stitch one, and make peace with your turning chain. 🧶 Save this for your next project!' + HT + ' #crochettips #crochetforbeginners' },
  { id: 's_reel_handbags', type: 'reel', file: '/uploads/social/reel-handbags.mp4', caption: 'Every bag starts as a single loop of yarn. Hand-crocheted, one of a kind — when it’s gone, it’s gone. 👜 Which one is you?' + HT + ' #crochetbag #handmadebag #slowfashion' },
  { id: 's_tip06', type: 'image', file: '/uploads/social/tip-06.jpg', caption: 'Care note Nº06 — how to love a handmade piece. Cold water, gentle hands, dry flat in the shade. Treated kindly, handmade outlives trends. ♡' + HT + ' #handmadecare #crochetcare' },
  { id: 's_tip02', type: 'image', file: '/uploads/social/tip-02.jpg', caption: 'Maker note Nº02 — match the hook to the yarn. And the amigurumi secret: always go one size smaller than the label says. 🧶' + HT + ' #amigurumi #crochettips' },
  { id: 's_reel_baskets', type: 'reel', file: '/uploads/social/reel-baskets.mp4', caption: 'Cuddle baskets — a friend that keeps your things. Hooked stitch by stitch by Undraa. 🐻🦊🦄 Which one is you?' + HT + ' #amigurumi #crochetbasket #handmadegifts' },
  { id: 's_tip07', type: 'image', file: '/uploads/social/tip-07.jpg', caption: 'Honest note Nº07 — why handmade costs more. A basket is 4–6 hours of handwork by the same two hands. You’re not paying for a product; you’re paying for time, skill, and a piece that exists once. ✨' + HT + ' #supportsmallbusiness #whyhandmade' },
  { id: 's_tip03', type: 'image', file: '/uploads/social/tip-03.jpg', caption: 'Maker note Nº03 — the magic ring in four steps. No more holes in your amigurumi! Save this one. 🧶' + HT + ' #magicring #amigurumitutorial #crochetforbeginners' },
  { id: 's_tip08', type: 'image', file: '/uploads/social/tip-08.jpg', caption: 'Gift note Nº08 — the right crochet gift for every person. New home, new baby, or just because — handmade says what cards cannot. 🎁' + HT + ' #handmadegifts #giftideas' },
  { id: 's_reel_littlethings', type: 'reel', file: '/uploads/social/reel-littlethings.mp4', caption: 'Little things, big smiles — keychains from $3. Fish, croissants and pocket blessings, all hooked by hand. 🐟🥐' + HT + ' #crochetkeychain #handmadekeychain' },
  { id: 's_tip04', type: 'image', file: '/uploads/social/tip-04.jpg', caption: 'Maker note Nº04 — US or UK? One pattern, two languages. Check the origin before you start, and when in doubt, look at the photos. 🧶' + HT + ' #crochetpatterns #crochettips' },
  { id: 's_tip09', type: 'image', file: '/uploads/social/tip-09.jpg', caption: 'Care note Nº09 — keep your plushies plump. Fluff monthly, spot-clean faces, store them sitting up. Loved plushies live for decades. 🧸' + HT + ' #amigurumi #plushies' },
  { id: 's_tip05', type: 'image', file: '/uploads/social/tip-05.jpg', caption: 'Maker note Nº05 — tension is a feeling. Relax your hands, breathe out before every row. Your stitches know what kind of day you had. 🧶' + HT + ' #crochettherapy #crochettips' },
  { id: 's_tip10', type: 'image', file: '/uploads/social/tip-10.jpg', caption: 'Honest note Nº10 — how to spot true quality in crochet. Even stitches, invisible ends, seams that don’t shift. Look closely — good work invites it. ✨' + HT + ' #handmadequality #crochetlove' },
];

export function getSocial() {
  const db = read();
  return Array.isArray(db.social) && db.social.length ? db.social : SEED_SOCIAL.slice();
}
export function addSocial({ type, file, caption }) {
  const db = read();
  if (!Array.isArray(db.social) || !db.social.length) db.social = SEED_SOCIAL.slice();
  const item = { id: newId('s'), type: type === 'reel' ? 'reel' : 'image', file, caption: String(caption || '') };
  db.social.push(item);
  write(db);
  return item;
}
export function removeSocial(id) {
  const db = read();
  if (!Array.isArray(db.social) || !db.social.length) db.social = SEED_SOCIAL.slice();
  const before = db.social.length;
  db.social = db.social.filter(x => x.id !== id);
  write(db);
  return db.social.length < before;
}
...
export function updateSettings(patch) {
  ...
  if (patch.igSocialIdx !== undefined) s.igSocialIdx = Number(patch.igSocialIdx) || 0;
  if (patch.igLastType !== undefined) s.igLastType = String(patch.igLastType);
  write(db);
  return s;
}
